...
function cloneStatus(status){return status?JSON.parse(JSON.stringify(status)):status}
function setPendingUiAction(message){pendingUiAction=String(message||'');if(lastStatus){render(lastStatus)}}
...
function optimisticQueueErrorStatus(status,id,action){const next=cloneStatus(status)||{};next.errors=(next.errors||[]).map(item=>{if(item&&typeof item==='object'&&String(item.id||'')===String(id||'')){const updated=Object.assign({},item,{queued_action:action,actions:[]});return updated}return item});return next}
...
function saveSelectedFolders(){post('/api/folders',{folders:collectSelectedFolders()},{pendingMessage:'Saving folder selection…'})}
function addFolder(path){const current=collectSelectedFolders();if(current.some(item=>item.path===path)){return}current.push({path,media_type:guessMediaType(path)});post('/api/folders',{folders:current},{pendingMessage:'Adding folder…'})}
...
function selectBrowsedFolder(path){post('/api/custom-folder',{path:path,media_type:guessMediaType(path)},{pendingMessage:'Adding folder…'});closeFolderBrowser()}
...
function addManualFolder(){const input=document.getElementById('manualFolderPath');const path=(input&&input.value?input.value:'').trim();if(!path){return}post('/api/custom-folder',{path:path,media_type:guessMediaType(path)},{pendingMessage:'Adding folder…'});if(input){input.value=''}}
function removeFolder(path){post('/api/folders',{folders:collectSelectedFolders().filter(item=>item.path!==path)},{pendingMessage:'Removing folder…'})}
...
function triggerUpdate(){const button=document.getElementById('updateButton');if(button&&button.disabled){return}post('/api/update',{}, {pendingMessage:'Preparing update…'})}
