...
class ManagerNodeTests(unittest.TestCase):
    ...
    def test_manager_step_uses_track_policy_final_stream_counts(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            config = self.make_config(temp_dir)
            source = self.create_source_file(temp_dir)
            ready_dir = Path(config["shared_state_dir"]) / "ready_outputs" / "job_track_cleaned"
            ready_dir.mkdir(parents=True)
            (ready_dir / "output.mkv").write_text("dry-run output\n", encoding="utf-8")
            queue_store.write_json_atomic(ready_dir / "worker_log.json", {"job_id": "job_track_cleaned", "status": "ready_for_finalize"})
            (ready_dir / "checksum.sha256").write_text("deadbeef  output.mkv\n", encoding="utf-8")
            source_summary = {"media_type": "anime", "file_size_bytes": 1000, "duration_seconds": 120.0, "audio_stream_count": 9, "subtitle_stream_count": 8}
            output_summary = {"duration_seconds": 120.0, "audio_stream_count": 2, "subtitle_stream_count": 3, "video_codec": "hevc", "file_size_bytes": 15}
            track_policy = {"applied": True, "expected_audio_stream_count": 2, "expected_subtitle_stream_count": 3}
            queue_store.write_json_atomic(ready_dir / "manifest.json", {"job_id": "job_track_cleaned", "source_path": str(source), "ready_output_path": str(ready_dir / "output.mkv"), "dry_run": True, "source_summary": source_summary, "output_summary": {**source_summary, "video_codec": "hevc"}, "track_policy": track_policy})
            queue_store.write_json_atomic(ready_dir / "output.ffprobe.json", {"job_id": "job_track_cleaned", "source_path": str(source), "dry_run": True, "source_summary": source_summary, "output_summary": output_summary, "track_policy": track_policy})
            self.enqueue_ready_job(config, {"schema_version": 1, "job_id": "job_track_cleaned", "status": "ready_for_finalize", "source_path": str(source), "media_type": "anime", "bucket": "anime_high", "duration_seconds": 120.0, "audio_stream_count": 9, "subtitle_stream_count": 8, "source_size_bytes": 1000, "dry_run": True, "ready_output_dir": str(ready_dir), "ready_output_path": str(ready_dir / "output.mkv"), "ready_output_manifest": str(ready_dir / "manifest.json"), "ready_output_ffprobe": str(ready_dir / "output.ffprobe.json")})

            result = manager_node.manager_step(config, dry_run_result="done")
            done_job = queue_store.read_json(Path(result["job_path"]))
            status = queue_store.queue_status(config)
